...
export function SiteFooter({ initialSettings }: { initialSettings?: PublicStoreSettings | null }) {
  ...
  return <footer className="store-footer">
    <div className="store-footer-main">
      ...
      <nav className="store-footer-column" aria-label="Покупателям"><h2>Покупателям</h2><Link href="/catalog">Каталог</Link><Link href="/new">Новинки</Link><Link href="/favorites">Избранное</Link><Link href="/cart">Корзина</Link><Link href="/faq">Вопросы и ответы</Link></nav>
      <div className="store-footer-column"><h2>Всегда на связи</h2>{phone && <a href={`tel:${phone.replace(/[^\d+]/g, "")}`}>{phone}</a>}{telegram && <a href={telegramHref} target="_blank" rel="noopener noreferrer">Telegram</a>}{email && <a href={`mailto:${email}`}>{email}</a>}<Link href="/help">Написать в поддержку</Link><Link href="/profile">Личный кабинет</Link></div>
    </div>
