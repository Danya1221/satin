  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/20 p-5 sm:p-6">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <LibraryField label="Текст кнопки 1 (пусто — без кнопки)"><input value={banner.buttonText} onChange={(event) => updateBanner(banner.id, { buttonText: event.target.value })} className="admin-input" /></LibraryField>
        ...
        <LibraryField label="Где использовать"><select value={banner.placement} onChange={event => updateBanner(banner.id, { placement: event.target.value })} className="admin-input"><option value="home">Главный экран — слайд</option><option value="hero">Главный экран</option><option value="manual">Отдельный блок редактора</option><option value="catalog">Каталог</option><option value="product">Товар</option></select></LibraryField>
        ...
        <LibraryField label="Тон"><select value={banner.tone} onChange={(event) => updateBanner(banner.id, { tone: event.target.value })} className="admin-input"><option value="blue">Тёплый акцент</option><option value="dark">Тёмный</option><option value="light">Светлый</option></select></LibraryField>
