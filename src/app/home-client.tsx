...
export default function Home({ initialData = {} }: { initialData?: HomePayload }) {
  ...
  const blocks = (data.pageBlocks?.length ? data.pageBlocks : defaultBlocks)
...
function HomeModule({ block, data }: { block: HomePageBlock; data: HomePayload }) {
  ...
  if (type === "hero") return <Hero banners={banners} />;
  if (type === "benefits") return <Benefits benefits={(data.benefits ?? []).filter(benefit => benefit.enabled).slice(0, limit(settings, 4))} />;
  if (type === "categories" || type === "category-grid") {
    ...
    return <section className="store-section"><SectionHead eyebrow="Найдётся для каждого" title={text(settings, "title", "Что вам по душе?")} subtitle={text(settings, "subtitle")} action={settings.showButton !== false ? { text: text(settings, "buttonText", "Весь каталог"), href: text(settings, "buttonHref", "/catalog") } : undefined} />
      <div className="store-categories">{categories.map(category => <Link key={category.id} className="store-category" href={category.href || `/catalog/${category.slug || category.id}`}><div className="store-category-image">{category.image ? <StoreImage src={category.image} alt="" width={200} height={180} sizes="(max-width: 639px) 28vw, 180px" /> : <StoreIcon name="grid" />}</div><strong>{category.name}</strong></Link>)}</div>
    </section>;
  }
  ...
  if (type === "new-arrivals") {
    ...
    return <section className="store-section"><SectionHead eyebrow="Новое и интересное" title={text(settings, "title", "Знакомьтесь. Ваши новые любимые.")} subtitle={text(settings, "subtitle")} action={{ text: "Все новинки", href: "/new" }} />
  ...
  if (type === "promo-banner") {
    const banner = text(settings, "bannerId") ? banners.find(item => item.id === settings.bannerId) : banners.find(item => item.placement === text(settings, "placement", "home")) ?? banners[0];
...
function SectionHead({ eyebrow, title, subtitle, action }: { eyebrow?: string; title: string; subtitle?: string; action?: { text: string; href: string } }) {
  return <div className="store-section-head"><div>{eyebrow && <span className="store-eyebrow">{eyebrow}</span>}<h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</div>{action && <Link href={action.href} className="store-text-link">{action.text.replace(/\s*[→➜➡]+\s*$/, "")}<StoreIcon name="arrow" /></Link>}</div>;
}

function Hero({ banners, secondary = false }: { banners: HomeBanner[]; secondary?: boolean }) {
  const { dark } = useTheme();
  ...
  return <section className="store-hero" data-editorial={image === "/images/technology-hero.webp"} data-layout={banner?.layout || "image-bg"} data-title-size={banner?.titleSize} data-text-size={banner?.textSize} aria-roledescription={slides.length > 1 ? "карусель" : undefined} aria-label={title || "Техника для вашего ритма жизни"} onTouchStart={event => { touchX.current = event.touches[0].clientX; }} onTouchEnd={event => { if (touchX.current !== null && Math.abs(touchX.current - event.changedTouches[0].clientX) > 65) next(touchX.current > event.changedTouches[0].clientX ? 1 : -1); touchX.current = null; }}>
    ...
    <div className="store-hero-content"><span className="store-eyebrow">{banner?.label || "Технологии. С характером."}</span>
      ...
      <p className="store-hero-description">{banner?.subtitle || banner?.description || "Для больших планов, любимой музыки и всего, что делает ваш день особенным."}</p>
      <div className="store-hero-actions"><Link href={banner?.buttonHref || "/catalog"} className="store-button">{banner?.buttonText || "Выбрать своё"}<StoreIcon name="arrow" /></Link>{banner?.secondaryButtonText ? <Link href={banner.secondaryButtonHref || "/help"} className="store-text-link">{banner.secondaryButtonText}</Link> : !banner && <Link href="/new" className="store-text-link">Смотреть новинки</Link>}</div>
    </div>
...
function Benefits({ benefits }: { benefits: HomeBenefit[] }) {
  ...
  return <div className="store-benefits" aria-label="Преимущества магазина">{benefits.map((benefit, index) => {
    ...
    return benefit.href ? <Link className="store-benefit" key={benefit.id} href={benefit.href}>{content}</Link> : <div className="store-benefit" key={benefit.id}>{content}</div>;
  })}</div>;
}
